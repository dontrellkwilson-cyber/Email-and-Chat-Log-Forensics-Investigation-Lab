...
This lab documents a forensic investigation of email and chat artifacts recovered from a Windows drive image and from imported cloud application data. Using Paraben’s E3, I examined Outlook and Thunderbird email databases, analyzed RFC email headers, sorted attachments, searched for targeted terms and emojis, OCR'd text in images, and reviewed Slack and Discord communications.
...
- RFC 822, RFC 2822, and RFC 5322 header review
- Attachment triage
...
## Finding 2: Sender IP Address from an RFC Header
...
I selected the **RFC Header** viewer and reviewed the message-routing and authentication fields. The header identified the sender IP address as:

```text
209.85.167.54
```

Email headers are important because each mail server can add routing information as a message travels to its destination. This information can help identify the originating infrastructure and evaluate possible spoofing.
...
<p align="center">
  <em>Figure 2: The sender IP address identified in the RFC header of the `Re: Intricate Solution Job Offer` email.</em>
</p>
...
The messages appeared mostly consistent with normal workplace communication. However, messages involving Beverly and Mr. Harris Malone, also referred to as Alan, appeared unusually informal and subservient. The messages also referenced an urgent deadline at midnight on April 26, 2021.
...
The search returned several relevant messages, including a final email from Beverly that referenced a **Big Boss**. Although the email was not independently conclusive, the wording was suspicious when considered with the other communications.
...
- `star dust` had a specific meaning to Beverly.
- Beverly preferred to move the conversation away from the company Slack workspace.
- Discord could contain more direct evidence.
...
## Finding 3: `Well, well, well` Email Header
...
|---|---|
| Sender email address | `ADD-FROM-YOUR-HEADER-SCREENSHOT` |
| Mail server name | `ADD-FROM-YOUR-HEADER-SCREENSHOT` |
| Mail server IP address | `ADD-FROM-YOUR-HEADER-SCREENSHOT` |
...
The message appeared to request different drugs using emojis, making Leo a possible buyer and potential witness.
...
| Investigative significance | Possible alias and direct reference to drug sales |
...
Lena requested pills using the pill emoji. Beverly replied with a reference to **Mrs. M**, which appeared to connect the Discord conversation to the Mrs. Maximoff identity found in the Thunderbird evidence.
...
| Field | Finding |
|---|---|
| Conversation participant | `JulyRiley_35` |`Beverly_G_2345632`
| Relevant message | `will you help me with 💊` |`yep, write to red.witch231@gmail.com; Tell her you are from me and be nice
| Investigative significance | Additional communication related to suspected drug trafficking |
...
|---|---|---|
| Outlook | Suspicious communication with Mr. Harris Malone and reference to a Big Boss | Suggests a possible collaborator and higher-level contact |
| Slack | `star dust` conversation and instruction to use Discord | Indicates coded language and movement to another platform |
| Thunderbird | `redwitch321@gmail.com` account and Natasha “Red” Maximoff identity | Establishes a secondary identity associated with Beverly |
| Thunderbird OCR | Image containing the word `pills` | Provides an explicit drug-related reference hidden inside an attachment |
| Thunderbird | Leo Deforest requesting items through emojis | Identifies a likely buyer or witness |
| Discord | Lena Goodwin requesting pills | Corroborates emoji-based drug communications |
| Discord | Beverly references `Mrs. M` | Connects Discord communications to the Maximoff identity |
| Outlook challenge | Pill-emoji email thread | Extends the pattern back to Beverly’s primary email account |
| Discord challenge | Additional suspicious conversation | Adds independent supporting chat evidence |
...
## Beverly Used More Than One Identity

The Thunderbird account `redwitch321@gmail.com` contained messages signed as Natasha “Red” Maximoff. The Discord reference to `Mrs. M` supported a possible connection between the identities.

## Emojis and Code Words Were Investigatively Significant

The maple leaf, pill, wine-glass, and cocktail-glass emojis were used as search terms. The Slack phrase `star dust` also appeared to have a coded meaning.
...
A complete forensic review should therefore combine database navigation, targeted searching, metadata review, attachment analysis, OCR, and cross-platform correlation.
...
| Thunderbird header analyzed | Successful |
